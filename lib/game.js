...
export function generateDeck() {
  const deck = [];
  // SkyJo deck: -2(5), 0(15), 1-12(10 each), -1(10)
  // Simplified standard: -5 to 12 with standard counts
  const counts = {
    '-5': 2, '-4': 2, '-3': 5, '-2': 5, '-1': 10,
    '0': 15,
...
export function calcScore(grid) {
  return grid.reduce((sum, c) => sum + (c.revealed ? c.value : 0), 0);
}
...
export function checkColumnMatches(grid) {
  // Grid is 3 rows x 4 cols, stored as [row0col0, row0col1, ... row2col3]
  // Actually SkyJo is 4 rows x 3 cols
  // Layout: 12 cards in 4 rows of 3 cols
  // col 0: indices 0, 3, 6, 9
  // col 1: indices 1, 4, 7, 10
  // col 2: indices 2, 5, 8, 11
  const newGrid = grid.map(c => ({ ...c }));
  let removed = false;
  
  for (let col = 0; col < 3; col++) {
    const indices = [col, col + 3, col + 6, col + 9];
